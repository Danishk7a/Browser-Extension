...
async function changeTheme() {
    ...
    let colorPalette = 0;
   await fetch('http://localhost:3000/generatePalatte')
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        console.log('Success:', data.arr);


        colorPalette = rgbToHex(data.arr)
            ...
            if (colorMap[bgColor]) {
              ...
              chrome.storage.local.get('checkBtn', (result) => {
                ...
                if(checkbtn){
                  ...
                  // if(bgColor === 'rgb(255, 255, 255)' || 'rgb(0, 0, 0)'){
                  //   return
                  // }
...
document.addEventListener('keydown', (event) => {
  if (event.key === 'n') {
  document.getElementById('FontColors').click()
  }
});
